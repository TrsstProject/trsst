...
public interface Storage {
	...
	/**
	 * Returns the total number of entries for the specified feed id, or -1
	 * if the feed id is unrecognized or unsupported.
	 * 
	 * @param feedId
	 *            the specified feed.
	 ...
	 */
	String[] getEntryIdsForFeedId(String feedId, int start, int length,
	 ...
	 * associated with the specified feed. The retured string contains a
	 * signed feed element and holds all meta-data attributes associated with
	 * the feed. These contents may be inspected, analyzed, and indexed, but
	 * must be returned unmodifed to callers of readFeed() so the signature
	 * remains intact. Note that the feed element DOES NOT contain entry
	 * elements.
	 * 
	 * @param feedId
	 *            the specified feed.
	 ...
	 */
	String readEntry(String feedId, String entryId)
	 ...
	 * retrieval by readFeedEntry(). These contents may be inspected,
	 * analyzed, and indexed, but must be returned unmodifed to callers of
	 * readEntry() so the signature remains intact.
	 * 
	 * @param feedId
	 *            the specified feed.
	 * @param entryId
	 ...
	 *             if a error occurs persisting the entry data.
	 */
	void updateEntry(String feedId, String entryId, Date publishDate,
	 ...
	 */
	void deleteEntry(String feedId, String entryId)
			throws FileNotFoundException, IOException;

	/**
	 * Returns the mime-type of the contents of the resource data for the
	 * specified entry for the specified feed, if known. If not known,
	 * returns null.
	 * 
	 * @param feedId
	 *            the specified feed.
	 ...
	 */
	String readFeedEntryResourceType(String feedId, String entryId,
	 ...
	 *             if a error occurs obtaining the resource data.
	 */
	InputStream readFeedEntryResource(String feedId, String entryId,
			String resourceId) throws FileNotFoundException, IOException;

	/**
	 * Stores a binary resource for the specified feed and entry by reading
	 * the specified input stream and persisting the contents for later
	 * retrieval by readFeedEntryResource(). Implementors must close the
	 ...
	 */
	void updateFeedEntryResource(String feedId, String entryId,
	 ...
	 */
	void deleteFeedEntryResource(String feedId, String entryId,
